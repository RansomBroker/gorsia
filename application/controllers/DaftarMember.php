...
class DaftarMember extends CI_Controller
{
        ...
        if ($simpan) {
            ...
            for ($i = 0; $i < $totalDurasi; $i++) {
                ...
                $memberDetail = $this->db
                    ->query("SELECT * FROM member_trx where id='" . $simpan['idSimpanDetail'] . "'")
                    ->row_array();
                $member = $this->db
                    ->query("SELECT * FROM member where id='" . $memberDetail['memberID'] . "'")
                    ->row_array();
                // var_dump($simpan);
                ...
                #kategori or
                $rows_kat = $this->db
                    ->query("SELECT * FROM kategori_olahraga where id_kategori_olahraga='3'")
                    ->row_array();
                ...
                if ($totalDurasi > 1) {
                    ...
                    $this->M_penyewaan->insert_data_detail_jurnal(
                        $simpan_data_jurnal_detail,
                        $no_bukti,
                        $kodeTransaksi
                    );
    ...
    {
        $data['nota'] = $this->M_daftar_member->getNotaUtama($id_transaksi);
        $this->load->view('v_member_cetak_nota', $data);
    }
}
