...
class M_kode_akuntansi extends CI_Model
{
  ...
  public function get_all_akun_child()
  {
    $table= "kode_akuntansi"; #nama_table
    $this->db->order_by("kode_akun", "asc"); #sortir_data
    $this->db->where('kode_parent', "-");
    $query = $this->db->get($table);
    return $query->result();
  }
  ...
  public function get_all_akun_by_level($level)
  {
    $table= "kode_akuntansi"; #nama_table
    $this->db->where("level", $level); #sortir_data
    $this->db->where_in('kode_parent', ['11','12']);
    $this->db->order_by("kode_akun", "asc"); #sortir_data
    $query = $this->db->get($table);
    return $query->result();
  }

  public function get_all_akun_by_kode_parent($kode_parent)
  {
    $table= "kode_akuntansi"; #nama_table
    $this->db->order_by("kode_akun", "asc"); #sortir_data
    $this->db->where('kode_parent', $kode_parent);
    $query = $this->db->get($table);
    return $query->result();
  }
