...
<body>
      ...
      <div class="layout-page">
        ...
        <div class="content-wrapper">
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Form Data /</span> Check In</h4>
            <div class="card">
              <h5 class="card-header">Data Check In</h5>
              ...
              <div class="row">
                <div class="col-xl">
                  <div class="card mb-12">
                    <div class="card-body">
                      <form method="post" action="<?= base_url() ?>?/CheckInOut/store">
                        <div class="row">
                          <div class="col-6">
                            <div class="mb-3">
                              <label class="form-label" for="basic-default-fullname">Kode Member</label>
                              <select class="form-select select2" id="kodeMember" name="kodeMember" aria-label="Default select example" onchange="getDetailMember()" required>
                                <option value="" selected>- - - Pilih - - -</option>
                                <?php foreach($get_all_member as $row) {  
                                  print "<option value='$row->kodeMember' data-nama='$row->nama' data-telepon='$row->nope'>$row->kodeMember - $row->nama - $row->nope</option>";
                                } ?>
                              </select>
                            </div>
                            <div class="mb-3">
                              <label class="form-label" for="basic-default-fullname">Nama Pelanggan</label>
                              <input type="text" class="form-control" id="nama" name="nama" placeholder="Isi nama pelanggan" readonly required />
                            </div>

                            <div class="mb-3">
                              <label class="form-label" for="basic-default-fullname">No. Telepon</label>
                              <input type="text" class="form-control" id="no_telepon" name="no_telepon" placeholder="Isi no. telepon" readonly required />
                            </div>

                            <div class="mb-3">
                              <label class="form-label" for="basic-default-fullname">Check In</label>
                              <input type="time" class="form-control" id="checkIn" name="checkIn" placeholder="Check In" required />
                              <p style="color: red;">Kuota Check In sisa: <?php echo ($totalMaxCheckIn-$totalCheckIn); ?></p>
                            </div>
                          </div>
                        </div>
                        <?php if ($totalCheckIn==$totalMaxCheckIn) { ?>
                          <button type="submit" class="btn btn-primary" disabled>Submit</button>
                          <p style="color: red;">Kuota Fitness Maksimal <?php echo $totalMaxCheckIn; ?></p>
                        <?php } else{ ?>
  ...
  <script type="text/javascript">
    ...
    function getDetailMember(){
      let selected = $('#kodeMember').find('option:selected');    
      console.log(selected);
      if (selected) {
        document.getElementById("nama").value = selected.data('nama');
        document.getElementById("no_telepon").value = selected.data('telepon');
      }
    }
  </script>
